;; Original file from technomancy
...
(defvar ruby-compilation-executable "/usr/bin/ruby")
...
(defun ruby-test-one ()
  ...
  (let* ((funname (which-function))
    ...
    (compile (concat  ruby-compilation-executable " " buffer-file-name " --name " fn))))



;; test selenium or regular test
(defun ruby-test-file ()
  (interactive)
  (if (or (string-match "_sel.rb$" buffer-file-name) (string-match "_spec.rb$" buffer-file-name) (string-match "_test.rb$" buffer-file-name))
      (compile (concat ruby-compilation-executable " " buffer-file-name))
    (toggle-buffer)
    (compile (concat ruby-compilation-executable " " buffer-file-name))
    (toggle-buffer)))
...
(define-key ruby-mode-map [f5] 'ruby-test-file)
